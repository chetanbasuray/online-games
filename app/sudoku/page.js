...
import FloatingBubbles from "../components/FloatingBubbles"; // import reusable component
...
export default function SudokuPage() {
  ...
  const [tempGlow, setTempGlow] = useState({});
  ...
  const checkCompletion = (board,row,col)=>{
    ...
    if(board[row].every(n=>n!==0) && board[row].every((n,i)=>n===solution[row][i])){
      if(!completedRows.includes(row)){ rowDone=true; setCompletedRows(prev=>[...prev,row]); }
    }
    ...
    if(colVals.every(n=>n!==0) && colVals.every((n,i)=>n===solutionCol[i])){
      if(!completedCols.includes(col)){ colDone=true; setCompletedCols(prev=>[...prev,col]); }
    }
    ...
    if(blockVals.every(n=>n!==0) && blockVals.every((n,i)=>n===solutionBlock[i])){
      if(!completedBlocks.includes(blockKey)){ blockDone=true; setCompletedBlocks(prev=>[...prev,blockKey]); }
    }
  ...
  const getCellGlowStyle = (r,c)=>{ const key=`${r}-${c}`; return tempGlow[key]?"brightness-125 scale-105 transition-all duration-500":""; };
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center p-4 overflow-hidden bg-gradient-to-r from-purple-500 via-pink-500 to-red-500">
      ...
      <h1 className="text-4xl sm:text-5xl font-bold text-white mb-4 z-10">Sudoku</h1>
      ...
      {/* Sudoku Board */}
      <div className="grid grid-cols-9 gap-1 bg-white rounded-lg p-2 z-10">
        {userBoard.map((row,r)=>
          row.map((num,c)=>{
      ...
      {message && <motion.p initial={{opacity:0}} animate={{opacity:1}} transition={{duration:0.5}}
        className="mt-4 text-white font-bold text-lg z-10">{message}</motion.p>}
    </div>
  );
}
